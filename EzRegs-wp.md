...
GPT 甚至还给了份代码，虽然它给出了错误的生成多项式，但是这个时候聪明的解题人应该已经发现超出范围的值和实际值的异或的结果是一个定值。
...
= sum[2] ^ (sum[1] & sum[0])
```
感兴趣的话可以继续推导其余的 `l_c[3]~l_c[7]`，这里直接列出全部的式子（相信聪明的解题人也已经猜到规律了）：
```
...
注意到除法竖式里面除数最高位的系数“1”没染色，事实上这个 1 对应的是计算 top_line 时候的 op4 对应的一个常数系数。但是聪明的解题人可能要问了，这里是不是少了一个除法模块？因为做多项式除法的时候，当前位的商应该是当前余数最高次项系数**除以**除式的最高次项系数。除以一个数，相当于乘以这个数的逆，而无论是在有限域还是实数域，1 的逆都是 1，因此这里看似是一个无效的乘法，实际上前面应该还有个求逆的电路被优化掉了。
